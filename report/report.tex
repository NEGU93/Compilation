...
\begin{document}
\title{Building a Compilator for Java code}
...
"Optimizing compilers are so difficult to get right that we dare say that no optimizing compiler is error-free! Thus, the most important objective in writing a compiler is that it is correct"\cite{DRAGON_BOOK}
...
A lexical analyzer (\textit{Lexer}) is the first front-end step in compilers, matching keywords, comments, operators, etc, and generating a token stream for parsers called \textit{lexemes}. The Lexer reads input from the programming language to compile (mini c in our case) and matches it against regular expressions and runs a corresponding action if such an expression is matched.

To make the Lexer are going to use:
\begin{itemize}
	\item Regular Expresions: To describe the lexemes
	\item Finite Automata: To recongnize the expresions
\end{itemize}
\subsection{Regular Expresions}
...
The \textit{syntax analyzer's} job is to detect error of syntax and display the error that corresponds.  
\section{Semantic Analyzer}
\textit{"Well typed programs do not go wrong"}

A \textit{Semantic Analyzer} uses the syntax tree and the information in the symbol table to check the source program for semantic consistency with the language definition. 
